...
<div class="container-fluid premium-layout">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 fade-in-up" style="animation-delay: 0.1s;">
        <div>
            ...
            <p class="text-muted fw-medium mb-0 ms-5 ps-2">Sửa thông tin sản phẩm: <strong class="text-main">#{{ $product->id }} - {{ $product->name }}</strong></p>
        </div>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="fade-in-up" style="animation-delay: 0.2s;" id="productForm">
        @csrf
        @method('PUT')
        <div class="row g-4">
                    ...
                    <div class="card-body p-4 p-md-5">
                        ...
                        <div class="row">
                            <div class="col-md-8 mb-4">
                                ...
                                <input type="text" class="form-control form-control-lg premium-input @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $product->name) }}" required>
                            ...
                            <div class="col-md-4 mb-4">
                                ...
                                <input type="text" class="form-control form-control-lg premium-input text-uppercase @error('sku') is-invalid @enderror" id="base_sku" name="sku" value="{{ old('sku', $product->sku) }}" placeholder="VD: VOT-JOOLA">
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select form-select-lg premium-input @error('category_id') is-invalid @enderror" id="category_id" name="category_id" required>
                                ...
                                @foreach($categories as $parent)
                                    <option value="{{ $parent->id }}" class="fw-bold" {{ old('category_id', $product->category_id) == $parent->id ? 'selected' : '' }}>
                                        ❖ {{ $parent->name }}
                                    </option>
                        ...
                        <div class="mb-0">
                            ...
                            <textarea class="form-control premium-input" id="description" name="description" rows="6">{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                @php
                    ...
                    if($hasVariants) {
                        foreach($product->variants as $v) {
                            foreach($v->attributeValues as $av) { $existingAttrValIds[] = $av->id; }
                        }
                        $existingAttrValIds = array_unique($existingAttrValIds);
                    }
                @endphp
                ...
                <div class="card premium-card border-0 shadow-sm mb-4">
                    <div class="card-header bg-transparent border-bottom-0 pt-4 pb-2 px-4 p-md-5 d-flex justify-content-between align-items-center">
                        ...
                        <div class="form-check form-switch fs-4 mb-0">
                            <input class="form-check-input shadow-sm" type="checkbox" id="hasVariantsToggle" name="has_variants" value="1" {{ $hasVariants ? 'checked' : '' }} style="cursor: pointer;">
                        </div>
                    </div>
                    
                    <div class="card-body px-4 p-md-5 pt-0 {{ $hasVariants ? '' : 'd-none' }}" id="variantsSection">
                                            ...
                                            @foreach($attr->values as $val)
                                                <div class="form-check custom-checkbox d-flex align-items-center">
                                                    <input class="form-check-input attr-checkbox me-2" type="checkbox" 
                                                           ...
                                                           {{ in_array($val->id, $existingAttrValIds) ? 'checked' : '' }}>
                        ...
                        <div class="table-responsive {{ $hasVariants ? '' : 'd-none' }} rounded-3 overflow-hidden custom-scrollbar" id="variantTableContainer" style="border: 1px solid var(--border-color);">
                                ...
                                <tbody id="variantTableBody">
                                    @if($hasVariants)
                                        @foreach($product->variants as $index => $variant)
                                            @php $names = $variant->attributeValues->pluck('value')->implode(' - '); @endphp
                                            <tr>
                                                <td class="fw-bold text-primary ps-3" style="font-size: 0.9rem;">
                                                    {{ $names }}
                                                    <input type="hidden" name="variants[{{ $index }}][id]" value="{{ $variant->id }}">
                                                    @foreach($variant->attributeValues as $attrVal)
                                                        <input type="hidden" name="variants[{{ $index }}][attribute_values][]" value="{{ $attrVal->id }}">
                                                    @endforeach
                                                </td>
                                                <td><input type="text" name="variants[{{ $index }}][sku]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input text-uppercase fw-bold" style="font-size:0.85rem;" value="{{ $variant->sku }}" placeholder="Mã SKU..."></td>
                                                {{-- ĐÃ FIX LỖI HTML5 DECIMAL STEP BẰNG CÁCH DÙNG intval() --}}
                                                <td><input type="number" step="any" name="variants[{{ $index }}][price]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input" style="font-size:0.9rem;" value="{{ intval($variant->price) }}" required></td>
                                                <td><input type="number" step="any" name="variants[{{ $index }}][sale_price]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input text-danger fw-bold" style="font-size:0.9rem;" value="{{ $variant->sale_price ? intval($variant->sale_price) : '' }}" placeholder="Số tiền giảm"></td>
                                                <td><input type="number" name="variants[{{ $index }}][stock]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input" style="font-size:0.9rem;" value="{{ $variant->stock }}" required></td>
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4 position-relative z-index-0">
                            ...
                            <div class="input-group input-group-lg">
                                <input type="number" step="any" class="form-control premium-input @error('price') is-invalid @enderror" id="base_price" name="price" value="{{ old('price', $product->price == 0 ? intval($product->variants->max('price') ?? 0) : intval($product->price)) }}" {{ $hasVariants ? '' : 'required' }}>
                            ...
                            <small class="text-muted mt-1 d-block" style="font-size: 0.8rem;">Nhập giá này để áp dụng nhanh cho các biến thể.</small>
                        </div>

                        <div class="mb-4 position-relative z-index-0">
                            <label for="sale_price" class="form-label fw-bold text-main">Giá khuyến mãi</label>
                            <div class="input-group input-group-lg">
                                <input type="number" step="any" class="form-control premium-input @error('sale_price') is-invalid @enderror" id="base_sale_price" name="sale_price" value="{{ old('sale_price', $product->sale_price ? intval($product->sale_price) : '') }}" placeholder="0" min="0">
                                <span class="input-group-text fw-bold text-muted span-addon">đ</span>
                            </div>
                        </div>
                        ...
                        <div class="mb-4 position-relative z-index-0">
                            ...
                            <input type="number" class="form-control premium-input @error('stock') is-invalid @enderror" id="base_stock" name="stock" value="{{ old('stock', $product->stock) }}" {{ $hasVariants ? '' : 'required' }}>
                        </div>
                        ...
                        <div class="mb-0">
                            ...
                            <div class="d-flex gap-3">
                                <div class="form-check custom-radio">
                                    <input class="form-check-input" type="radio" name="status" id="status_1" value="1" {{ old('status', $product->status) == '1' ? 'checked' : '' }}>
                                ...
                                <div class="form-check custom-radio">
                                    <input class="form-check-input" type="radio" name="status" id="status_0" value="0" {{ old('status', $product->status) == '0' ? 'checked' : '' }}>
                ...
                <div class="card premium-card border-0 shadow-sm mb-4">
                    <div class="card-body p-4">
                        ...
                        @if($product->images->count() > 0)
                            <div class="mb-3">
                                <label class="fw-bold small text-muted mb-2">Ảnh hiện tại:</label>
                                <div class="row g-2" id="existing-images-container">
                                    @foreach($product->images as $img)
                                        <div class="col-4 position-relative" id="existing-img-box-{{ $img->id }}">
                                            <img src="{{ asset($img->image_path) }}" class="img-fluid rounded-3 shadow-sm border preview-img-obj">
                                            <button type="button" class="btn btn-danger btn-sm position-absolute rounded-circle remove-multi-btn shadow-sm" onclick="deleteServerImage({{ $img->id }})">
                        ...
                        <label class="fw-bold small text-muted mb-2">Thêm ảnh mới:</label>
                        <div class="upload-box shadow-sm mb-3" id="upload-box" onclick="document.getElementById('images-input').click()">
                            <div class="upload-content text-center">
                                <div class="icon-bg mb-3 mx-auto"><i class="fa-solid fa-cloud-arrow-up text-primary fs-3"></i></div>
                                <h6 class="fw-bold mb-1 text-main">Nhấn để chọn thêm ảnh</h6>
                            </div>
                        </div>
                ...
                <div class="d-grid gap-3">
                    <button type="submit" class="btn btn-warning btn-lg fw-bold text-dark shadow-sm hover-lift" style="border-radius: 12px;">
...
<script>
    // 1. AJAX Gọi xóa ảnh trên Server
    function deleteServerImage(imageId) {
        Swal.fire({
            title: 'Xóa ảnh này?', text: 'Ảnh sẽ bị xóa ngay lập tức!', icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                .then(data => {
                    if(data.success) {
                        document.getElementById('existing-img-box-' + imageId).remove();
                    }
                });
            }
        });
    ...
    // 2. Upload ảnh mới (Local preview)
        ...
        if (input.files) {
            for(let i = 0; i < input.files.length; i++) {
                ...
                reader.onload = function(e) {
                    ...
                    col.className = 'col-4 position-relative preview-item';
    ...
    document.getElementById('hasVariantsToggle').addEventListener('change', function() {
        ...
        if(this.checked) {
            ...
            basePrice.removeAttribute('required'); baseStock.removeAttribute('required');
        } else {
            section.classList.add('d-none');
            document.getElementById('variantTableContainer').classList.add('d-none');
            basePrice.setAttribute('required', 'true'); baseStock.setAttribute('required', 'true');
        }
    });

    // 4. Sinh lại biến thể (GIỮ LẠI ID CŨ ĐỂ KHÔNG BỊ LỖI KHI LƯU)
    function generateVariants() {
        ...
        // BƯỚC QUAN TRỌNG: Lưu lại ID và giá trị của bảng hiện tại
        const existingData = {};
        document.querySelectorAll('#variantTableBody tr').forEach(tr => {
            let idInput = tr.querySelector('input[name$="[id]"]');
            if(idInput) {
                let attrInputs = tr.querySelectorAll('input[name*="[attribute_values][]"]');
                let attrKey = Array.from(attrInputs).map(inp => inp.value).sort().join('-');
                
                existingData[attrKey] = {
                    id: idInput.value,
                    sku: tr.querySelector('input[name$="[sku]"]').value,
        ...
        combinations.forEach((combo, index) => {
            ...
            const attrKey = combo.map(c => c.id).sort().join('-');
            const oldData = existingData[attrKey];

            const idHTML = oldData ? `<input type="hidden" name="variants[${index}][id]" value="${oldData.id}">` : '';
            const skuVal = autoSku; 
            const priceVal = oldData ? oldData.price : currentBasePrice;
            const salePriceVal = oldData ? oldData.sale_price : currentBaseSalePrice;
            const stockVal = oldData ? oldData.stock : (currentBaseStock || 0);
            ...
            tr.innerHTML = `
                <td class="fw-bold text-primary ps-3" style="font-size: 0.9rem;">
                    ${names} ${idHTML} ${idsHTML}
                </td>
                <td><input type="text" name="variants[${index}][sku]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input text-uppercase fw-bold" style="font-size:0.85rem;" value="${skuVal}" placeholder="Mã SKU..."></td>
                <td><input type="number" step="any" name="variants[${index}][price]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input" style="font-size:0.9rem;" value="${priceVal}" placeholder="0" required></td>
                <td><input type="number" step="any" name="variants[${index}][sale_price]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input text-danger fw-bold" style="font-size:0.9rem;" value="${salePriceVal}" placeholder="Ko giảm"></td>
                <td><input type="number" name="variants[${index}][stock]" class="form-control rounded-3 py-1 px-2 text-center inner-table-input" style="font-size:0.9rem;" value="${stockVal}" required></td>
                <td class="text-center pe-3"><button type="button" class="btn btn-sm btn-light text-danger rounded-circle border shadow-sm p-1 d-flex align-items-center justify-content-center mx-auto" style="width:28px; height:28px;" onclick="this.closest('tr').remove()"><i class="fa-solid fa-xmark"></i></button></td>
            `;
